...
void untitled_step(void)
{
  ...
  untitled_B.PulseGenerator = ((untitled_DW.clockTickCounter < 50000) &&
  ...
  if (untitled_B.PulseGenerator != TIM1_DataLink.TIM_Freq) {
    TIM1_DataLink.TIM_Freq = untitled_B.PulseGenerator;

    //Update channel4 duty cycle
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_4, (uint32_t)
                         (TIM1_DataLink.CH4_duty * (100000.0 /
      untitled_B.PulseGenerator - 1) / 100));

    //Update frequency
    __HAL_TIM_SetAutoreload(&htim1, 100000.0 / untitled_B.PulseGenerator - 1);
  }
}
